Stats_fun_v5_only_dic <- function (df){
  # This function relies on the samples to be introduced in the data as Sample_ID
 if ("DIC_mg_C_per_L"%in% colnames(df) == TRUE){
   # Adding a column for just sample names, no reps
   data.stats = as.data.frame(matrix(NA,ncol = 6, nrow = nrow(df)))
   colnames(data.stats) = c("Sample_ID","Sample_name","DIC_CV","DIC_variance","DIC_range","DIC_normalized_range")
   ...
   for (i in 1:length(unique.sample.names)){
     ...
     DIC.temp = df$DIC_mg_C_per_L[grep(unique.sample.names[i],data.stats$Sample_name)]
     
     data.stats$DIC_CV[index] = sd(DIC.temp) / mean(DIC.temp) * 100
     data.stats$DIC_range[index] =  max(DIC.temp) - min(DIC.temp)
     data.stats$DIC_variance[index] = var(DIC.temp)
     data.stats$DIC_normalized_range[index] = (max(DIC.temp) - min(DIC.temp))/mean(DIC.temp)
     
   }
   
   hist(data.stats$DIC_CV, breaks = 55)
  # hist(data.stats$DIC_range, breaks = 55)
   #hist(data.stats$DIC_variance, breaks = 55)
